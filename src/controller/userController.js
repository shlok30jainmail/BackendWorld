...
} from "../helpers/generateRandomOTPorReferral.js";
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      if (!name || !email) {
        return res.status(500).json({
          success: false,
          message:
            "You are new candidate so we need your name and email also to register you !",
        });
      }
    ...
    res.status(200).json({
      ...
      message:
        email !== undefined
          ? "OTP successfully sent on your gmail"
          : "OTP successfully sent on your mobile",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const getAllUser = async (req, res) => {
  ...
  const query = {
    ...
    ...(search && {
      // when we use multiple fields using one searcg
      $or: [
